		...
		if (web3.isConnected()) {
			...
			if (certificates.length > 0) {
				certificates.forEach(function (index) {
					...
					$('table tbody').append('<tr><td>' + student + '</td><td>' + certificateDetails[2] + '</td><td>' + certificateDetails[3] + '</td><td>');
					$('table').show();
				});
			} else {
				$('table').replaceWith('<div class="alert alert-warning">No issued LORs</div>');
			}
