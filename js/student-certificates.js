requireJS.require(['settings', 'web3', 'jquery', 'bootstrap'], function (settings) {
	...
	$(document).ready(function () {
		...
		if (web3.isConnected()) {
			...
			console.log(certificates.length + " LOR record(s) loaded");
			...
			if (certificates.length > 0) {
				certificates.forEach(function (index) {
					...
					$('table tbody').append('<tr><td>' + issuer + '</td><td>' + certificateDetails[2] + '</td><td>' + certificateDetails[3] + '</td><td>');
					$('table').show();
				});
			} else {
				$('table').replaceWith('<div class="alert alert-warning">You have no LOR.</div>');
			}
